...
public class PlayerActivityDetailsMetrics  implements Configurable, IJobFactory {
	public PlayerActivityDetailsMetrics(Configuration conf) {
		setConf(conf);
	}

	@Override
	public Job createJob() throws IOException {
		Job job = Job.getInstance(getConf(), "PlayerActivityDetails metrics");
		...
		final Path outPath = (new Path(getConf().get("com.hadoopexamples.pokerstatistics.out"))).suffix("/playeractivitydetails");
