...
public class RGPlayers implements IMetric {
	...
	private static final String m_name = "RGPlayers";
	...
	public static class Mapper implements IMetricMapper {
		...
		public void map(final Hand hand, IMapperSink sink) throws IOException, InterruptedException {
			...
			if (count != 0) {
				sink.write(m_name, TagsMaker.makeTags(hand), MetricValue.newBuilder().setIntValue(count).build());
			}
		}
	}
